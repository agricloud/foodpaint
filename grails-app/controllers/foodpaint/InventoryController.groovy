...
class InventoryController {
    ...
    def update(){
        ...
        if(!InventoryDetail.findByWarehouseAndItem(inventory.warehouse,inventory.item)){
            if(inventoryService.consume(inventory.warehouse.id, inventory.item.id, inventory.qty).success){
                if(inventoryService.replenish(params.warehouse.id, params.item.id, params.qty.toLong()).success){
                    render (contentType: 'application/json') {
